...
const ENDPOINT = 'nam-tuyen-sinh';
const columns: CatalogColumn[] = [{ key: 'nam', label: 'Năm' }];
const fields: CatalogField[] = [{ key: 'nam', label: 'Năm', type: 'text' }];

export default function NamTuyenSinhPage() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      setError('Không thể tải danh sách năm tuyển sinh');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div>
      ...
      <CatalogForm
        ...
        fields={fields}
        initialValues={editingItem ? { nam: editingItem['nam'] } : undefined}
        onSubmit={editingItem ? handleEdit : handleAdd}
        onCancel={editingItem ? () => setEditingItem(null) : undefined}
        isEditing={!!editingItem}
        title={editingItem ? 'Cập nhật năm tuyển sinh' : 'Thêm năm tuyển sinh'}
      />
      ...
      <CatalogTable
        ...
        title="Danh sách năm tuyển sinh"
        emptyMessage="Chưa có năm tuyển sinh nào"
      />
    </div>
  );
}
